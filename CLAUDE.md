...
- `Lexer` — tokenizer, raises `Lex_error` on invalid input. Supports Unicode identifiers and unit suffixes (non-ASCII bytes accepted). Column positions track codepoints, not bytes (via `String.get_utf_8_uchar`). Tokens include `DOUBLE_COLON` (`::`) and `ARROW` (`->`); reserved keywords (`let`, `loop`, `in`) are recognized by `read_ident` after reading the full identifier. `read_ident` uses lookahead to stop before `->` so that `A->B` tokenizes correctly despite `-` being a valid identifier character.
...
- `parser.ml`: Comments on `Var`, `App`, `Lambda`, `Let`, and `Unit` nodes are silently dropped at parse time (`attach_comments_right` treats them as leaves). See Future Ideas for candidate designs.
